  ...
  return (
    ...
    >
      <div className="container-padding py-3 sm:py-4">
        {/* Mobile: Side-by-side layout with Add to Cart on left, Back on right */}
        <div className="flex sm:hidden items-center justify-between gap-2 px-4">
          <div className="flex-1">
            <motion.button
              onClick={onAddToCart}
              disabled={!product.inStock}
              className="btn-custom-color text-black w-full px-3 py-2 rounded-xl font-semibold flex items-center justify-center gap-1 text-xs hover:bg-yellow-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
          ...
          <div className="flex-1">
            <Link
              to="/products"
              className="bg-gray-600 text-white w-full px-3 py-2 rounded-xl font-semibold flex items-center justify-center gap-1 text-xs hover:bg-gray-700 transition-colors"
            >
...
const FloatingBackButton: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      transition={{ type: "spring", stiffness: 300, damping: 30 }}
      className="hidden sm:block bottom-5 right-4 z-50 fixed" // Hide on mobile
    >
      <Link
        to="/products"
        className="flex items-center gap-2 bg-yellow-600 text-white px-6 py-3 sm:px-4 sm:py-2 rounded-full shadow-lg hover:bg-yellow-700 transition-colors"
      >
        <ArrowLeft className="w-5 h-5" />
        <span className="text-md font-bold">Back to Products</span>
      </Link>
    </motion.div>
  );
};
  ...
  return (
    <ParallaxProvider>
      <div className="min-h-screen bg-gradient-to-b from-yellow-50 to-white">
        ...
        <section ref={heroRef} className="relative h-screen overflow-hidden pt-20 sm:pt-0"> {/* Added pt-20 for header clearance on mobile */}
          ...
          <motion.div
            ...
            className="absolute top-16 right-2 sm:right-20 w-12 h-12 sm:w-16 sm:h-16 bg-yellow-400/20 rounded-full backdrop-blur-sm" 
          />
          
          <motion.div
            animate={{ y: [0, 15, 0], rotate: [0, -3, 0] }}
            transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 2 }}
            className="absolute bottom-32 left-2 sm:left-16 w-10 h-10 sm:w-12 sm:h-12 bg-amber-400/20 rounded-full backdrop-blur-sm" 
          />

          {/* New Floating Element - Right Bottom - Adjusted */}
          <motion.div
            animate={{ y: [0, 10, 0], rotate: [0, 3, 0] }}
            transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }}
            className="absolute bottom-16 right-2 sm:right-16 w-10 h-10 sm:w-12 sm:h-12 bg-blue-400/20 rounded-full backdrop-blur-sm" 
          />
          ...
          >
            ...
            >
              ...
              <span className="text-lg sm:text-base">Back to Products</span>
            </Link>
          </motion.div>

          {/* Hero Content (inside glass card) - Improved mobile width/text */}
          <div className="absolute inset-0 flex items-center justify-center px-2 sm:px-4 overflow-hidden"> {/* Added overflow-hidden */}
            <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1 }}
              className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-8 md:p-12 shadow-lg w-full max-w-sm sm:max-w-4xl text-center text-white z-10 hyphens-auto"
            >
              <motion.h1
                initial={{ opacity: 0, y: 50 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 1, delay: 0.7 }}
                className="text-2xl sm:text-6xl md:text-8xl font-bold mb-3 sm:mb-6 text-shadow-lg leading-tight"
              >
                {product.name}
              </motion.h1>
              ...
              <motion.p
                ...
                className="text-base sm:text-2xl md:text-3xl mb-4 sm:mb-8 text-shadow leading-relaxed hyphens-auto" 
              >
                {product.description}
              </motion.p>
          ...
          {/* Scroll Indicator - Adjusted bottom for sticky bar */}
          <motion.div
  animate={{ y: [0, 10, 0] }}
  ...
  <div className="flex flex-col items-center">
    ...
    <div className="w-4 sm:w-6 h-6 sm:h-10 border-2 border-white rounded-full flex justify-center">
      <div className="w-1 sm:w-1 h-2 sm:h-3 bg-white rounded-full mt-1 sm:mt-2"></div>
        ...
        {/* [Omitted for brevity - include the full main content, sections, etc., from previous version] */}
...
<p className="text-neutral-600 text-sm sm:text-lg text-center sm:text-left">
  per {product.unit}
</p>
        ...
        {/* Sticky Add to Cart (now includes mobile back button) */}
        <StickyAddToCart
          product={product}
          quantity={quantity}
        ...
        {/* Floating Back to Products Button (desktop only) */}
        <FloatingBackButton />
      </div>
    </ParallaxProvider>
  );
